	...
	const ParamSet &tp)
{
	...
	if (!getline(fs, line).good()) {
		LOG(LUX_ERROR, LUX_BADFILE) << "Unable to read sopra file '" << filename << "'";
		return NULL;
	}
	...
	for (int i = count; i >= 0; --i) {
		...
		if (!boost::regex_search(line, m, sample_expr)) {
			LOG(LUX_ERROR, LUX_BADFILE) <<
				"Unparseable sopra data at data line " <<
				(count - i) << "' in '" << filename << "'";
			return NULL;
		}
	...
	const ParamSet &tp)
{
	...
	if (name == "amorphous carbon") {
		...
		if (name != "aluminium") {
			LOG(LUX_WARNING, LUX_BADTOKEN) << "Fresnel preset '" <<
				name << "' not found, using aluminium instead";
		}
		wl.assign(SopraWavelengths, SopraWavelengths + SopraSamples);
