...
class UniformBSDF : public BSDF
{
	public:
	UniformBSDF(
		const DifferentialGeometry &dgs, const Normal &ngeom,
	...
	virtual bool SampleF(
		const SpectrumWavelengths &sw, const Vector &woW,
	...
	virtual float Pdf(
		const SpectrumWavelengths &sw, const Vector &woW,
		const Vector &wiW, BxDFType flags = BSDF_ALL
	) const
	{
	...
	virtual SWCSpectrum F(
		const SpectrumWavelengths &sw, const Vector &woW,
		const Vector &wiW, bool reverse, BxDFType flags = BSDF_ALL
	) const
	{
		if (NumComponents(flags) == 1)
	...
	virtual SWCSpectrum rho(
		const SpectrumWavelengths &sw, BxDFType flags = BSDF_ALL
	) const
	{
	...
	virtual SWCSpectrum rho(
		const SpectrumWavelengths &sw, const Vector &woW, BxDFType flags = BSDF_ALL
	) const
	{
		return SWCSpectrum(1.f);
	};
...
) : Light("PointLight-" + boost::lexical_cast<string>(this), light2world), Lbase(L), gain(g), func(ssf)
{
...
bool PointLight::SampleL(
	...
	float *pdfDirect, SWCSpectrum *Le
) const
{
	const Normal ns(Normalize(LightToWorld * Normal(0, 0, 1)));
	DifferentialGeometry dg(lightPos, ns,
...
Light* PointLight::CreateLight(const Transform &light2world, const ParamSet &paramSet)
{
...
static DynamicLoader::RegisterLight<PointLight> rb("goniometric");
// Backwards compability for removed 'GonioPhotometricLight'
